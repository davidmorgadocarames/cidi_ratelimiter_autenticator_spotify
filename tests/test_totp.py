...
def _register_and_login(client: TestClient, email: str = "totp@example.com", password: str = PASSWORD):
...
def _setup(client: TestClient, access_token: str, password: str = PASSWORD):
    return client.post("/2fa/setup", json={"password": password}, headers=_headers(access_token))


def test_setup_requires_auth(client: TestClient):
    response = client.post("/2fa/setup", json={"password": PASSWORD})
    assert response.status_code == 401


def test_setup_rejects_wrong_password(client: TestClient):
...
def test_setup_returns_secret_and_qr(client: TestClient):
...
def test_setup_blocked_if_already_confirmed(client: TestClient, db_session):
    access_token = _register_and_login(client, email="doublesetup@example.com")
    secret = _setup(client, access_token).json()["secret"]

    code = pyotp.TOTP(secret).now()
    verify_response = client.post("/2fa/verify", json={"code": code}, headers=_headers(access_token))
...
def test_verify_without_setup_returns_400(client: TestClient):
    access_token = _register_and_login(client, email="noverify@example.com")
    response = client.post("/2fa/verify", json={"code": "123456"}, headers=_headers(access_token))
    assert response.status_code == 400


def test_verify_with_valid_code_confirms_setup(client: TestClient, db_session):
    access_token = _register_and_login(client, email="confirm@example.com")
    secret = _setup(client, access_token).json()["secret"]

    code = pyotp.TOTP(secret).now()
    response = client.post("/2fa/verify", json={"code": code}, headers=_headers(access_token))
    assert response.status_code == 200
...
def test_verify_when_already_confirmed_just_validates(client: TestClient):
    access_token = _register_and_login(client, email="already@example.com")
    secret = _setup(client, access_token).json()["secret"]
    totp = pyotp.TOTP(secret)
    client.post("/2fa/verify", json={"code": totp.now()}, headers=_headers(access_token))
    ...
    response = client.post("/2fa/verify", json={"code": later_code}, headers=_headers(access_token))
    assert response.status_code == 200
    assert response.json() == {"enabled": False, "valid": True}


def test_verify_with_incorrect_code_rejected(client: TestClient):
    access_token = _register_and_login(client, email="badcode@example.com")
    secret = _setup(client, access_token).json()["secret"]

    real_code = pyotp.TOTP(secret).now()
    wrong_code = "000000" if real_code != "000000" else "111111"
    response = client.post("/2fa/verify", json={"code": wrong_code}, headers=_headers(access_token))
    assert response.status_code == 401


def test_verify_within_tolerance_window_accepted(client: TestClient):
    ...
    response = client.post("/2fa/verify", json={"code": past_code}, headers=_headers(access_token))
    assert response.status_code == 200


def test_verify_future_within_tolerance_window_accepted(client: TestClient):
    ...
    response = client.post("/2fa/verify", json={"code": future_code}, headers=_headers(access_token))
    assert response.status_code == 200


def test_verify_outside_tolerance_window_rejected(client: TestClient):
    ...
    response = client.post("/2fa/verify", json={"code": expired_code}, headers=_headers(access_token))
    assert response.status_code == 401


def test_verify_future_outside_tolerance_window_rejected(client: TestClient):
...
def test_verify_rejects_malformed_code(client: TestClient):
    access_token = _register_and_login(client, email="malformed@example.com")
    _setup(client, access_token)
    response = client.post("/2fa/verify", json={"code": "abcdef"}, headers=_headers(access_token))
    assert response.status_code == 422


def test_setup_rejects_missing_password_field(client: TestClient):
...
def test_verify_lockout_after_five_failed_attempts(client: TestClient):
    ...
    locked_response = client.post("/2fa/verify", json={"code": real_code}, headers=_headers(access_token))
    assert locked_response.status_code == 429


def test_verify_success_resets_failed_attempts(client: TestClient, db_session):
    ...
    client.post("/2fa/verify", json={"code": wrong_code}, headers=_headers(access_token))
    client.post("/2fa/verify", json={"code": wrong_code}, headers=_headers(access_token))

    ok_response = client.post("/2fa/verify", json={"code": real_code}, headers=_headers(access_token))
...
def test_verify_concurrent_wrong_attempts_lockout_not_bypassed(client: TestClient):
...
def test_verify_handles_corrupted_secret_as_controlled_error(client: TestClient, db_session):
