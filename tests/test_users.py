...
def _enable_totp(
    client: TestClient, access_token: str, password: str = PASSWORD
) -> str:
    ...
    secret: str = setup_response.json()["secret"]
    code = pyotp.TOTP(secret).now()
    client.post("/2fa/verify", json={"code": code}, headers=_headers(access_token))
    return secret


def test_activate_premium_requires_auth(client: TestClient) -> None:
...
def test_activate_premium_blocked_without_2fa(client: TestClient) -> None:
...
def test_activate_premium_rejects_wrong_password(client: TestClient) -> None:
...
def test_activate_premium_rejects_wrong_totp_code(client: TestClient) -> None:
...
def test_activate_and_deactivate_premium_success(client: TestClient) -> None:
...
def test_deactivate_premium_does_not_require_2fa(client: TestClient) -> None:
...
def test_deactivate_premium_requires_auth(client: TestClient) -> None:
